...
async function uploadAchievements(path: string) {
	try {
		...
		achievementEntity.title = achievementData.title;
		achievementEntity.description = achievementData.description;
...
async function uploadProblems(path: string) {
	try {
		...
		for (var [problemName, problem] of Object.entries(problemData)) {
			...
			problemEntity.category = castProblem.tags[1];
			problemEntity.difficulty = castProblem.tags[0];
